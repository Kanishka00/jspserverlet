        ...
        <span class="inline-grid h-9 w-9 place-items-center rounded-xl bg-gradient-to-br from-brand-500 to-brand-700 text-white shadow-soft">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="h-5 w-5">
            ...
            <nav class="hidden md:flex items-center gap-8 text-sm">
                ...
                <% if (authType == null) { %>
                <a href="<%= request.getContextPath() %>/views/login.jsp" class="hover:text-brand-700">Login</a>
                <% } else if ("owner".equals(authType)) { %>
                <a href="<%= request.getContextPath() %>/views/user_appointments.jsp" class="hover:text-brand-700">My Appointments</a>
                <form method="post" action="<%= request.getContextPath() %>/logout">
                    <button class="hover:text-brand-700">Logout</button>
                </form>
                <% } else { %>
                <a href="<%= request.getContextPath() %>/views/receptionist-dashboard.jsp" class="hover:text-brand-700">Dashboard</a>
                <form method="post" action="<%= request.getContextPath() %>/logout">
                    <button class="hover:text-brand-700">Logout</button>
                </form>
                <% } %>
